...
std::vector<std::string> CommandLine::Parse(std::string Input, char Delimiter)
{
   ...
   bool InCustomQuote = false;

   int Start = 0;
   for(int i = 0; i < (int)Input.size(); i++)
   {
      if(InDoubleQuote == false && InSingleQuote == false && InCustomQuote == false)
      {
         if(Input[i] == '\'')
         {
            InSingleQuote = true;
      ...
      {
         if(InDoubleQuote == true && Input[i] == '\"')
         {
            InDoubleQuote = false;
            Input.erase(Input.begin() + i);
            i = i - 1;
         }
         if(InSingleQuote == true && Input[i] == '\'')
         {
            InSingleQuote = false;
            Input.erase(Input.begin() + i);
            i = i - 1;
         }
         if(InCustomQuote == true && Input[i] == '@')
         {
            InCustomQuote = false;
            Input.erase(Input.begin() + i);
            i = i - 1;
